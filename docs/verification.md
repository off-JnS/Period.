...
covered.** A number like "623 tests passing" says less than it appears to. What
...
**623 tests.** Not one number but three kinds, which fail for different reasons:
...
| Widget and integration tests | the rest of 623 | Screens render every state, the database round-trips, the backup survives export → wipe → import |
| Goldens | 42 images | What a person actually sees, in English and German, light and dark, at 100% and 200% text |
...
### Notifications are half-built, deliberately

`docs/cycle-logic.md` section 7 and the scheduling logic exist and are tested.
There is no plugin, no permission, no settings UI, and **no notification is ever
shown**. The logic has no caller but its tests.

Unknown, and unknowable here: whether a notification fires, survives a reboot,
holds its wall-clock time across a DST change, or is throttled by Doze.
...
3. **Finish notifications on a device**, where the questions the logic cannot
   answer become answerable.
